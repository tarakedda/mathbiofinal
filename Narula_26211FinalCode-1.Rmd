...
NHANES3_scaled <- NHANES3_imputed
for (col in biomarkers) {
  NHANES3_scaled[[col]] <- scale(NHANES3_scaled[[col]])
}

# lasso regression below

complete_idx <- complete.cases(NHANES3_scaled$kdm0, NHANES3_scaled[, biomarkers])
NHANES3_clean <- NHANES3_scaled[complete_idx, ]

x_train <- as.matrix(NHANES3_clean[, biomarkers])
y_train <- NHANES3_clean$kdm0  
...
cat("\nFull Linear Regression Model Summary:\n")
summary(full_model)
```


```{r}
top_4_biomarkers <- c("lymph", "mcv", "rdw", "wbc")  

reduced_model <- lm(kdm0 ~ ., data = NHANES4_scaled[, c("kdm0", top_4_biomarkers)])
cat("\nReduced Linear Regression Model Summary (Top 4 Biomarkers):\n")
...
cat("\nReduced Model AIC:", AIC(reduced_model), "\n")

# for lasso model AIC
biomarkers_for_lm <- selected_biomarkers[selected_biomarkers != "(Intercept)"]
lm_formula <- as.formula(paste("kdm0 ~", paste(biomarkers_for_lm, collapse = " + ")))
lasso_lm_model <- lm(lm_formula, data = NHANES3_clean)
cat("\nAIC for LASSO-selected linear model:", AIC(lasso_lm_model), "\n")
```

```{r}
hybrid_biomarkers <- unique(c(selected_biomarkers[1:3], top_4_biomarkers[1:2]))  # both lasso and linear

selected_biomarkers_clean <- selected_biomarkers[selected_biomarkers != "(Intercept)"]
hybrid_biomarkers <- unique(c(selected_biomarkers_clean[1:3], top_4_biomarkers[1:2]))
hybrid_model <- lm(kdm0 ~ ., data = NHANES4_scaled[, c("kdm0", hybrid_biomarkers)])
...
heatmap(cor_matrix, main = "Biomarker Correlation Heatmap", col = heat.colors(100))
```

```{r}
cor_matrix <- cor(NHANES4_scaled[, biomarkers])
heatmap(cor_matrix, main = "Top 4 Biomarker Correlation Heatmap", col = heat.colors(100))
```
...
cat("Hybrid Model R²:", summary(hybrid_model)$r.squared, "\n")
